"""CLI routing tests: no model downloads, GPU, or MLX installation required."""
...
def backends(monkeypatch):
    ...
    # A real two-argument signature detects stale dtype arguments to MLX.
...
@pytest.mark.parametrize("mode,flags", [
    ...
    ("direct", ["--device", "mps"]),
    ("direct", ["--device", "cuda"]),
    ("serial", ["--dtype", "float16"]),
    ("shared", ["--dtype", "float32"]),
])
def test_invalid_mlx_flags_fail_before_loading(run_cli, backends, capsys, mode, flags):
    with pytest.raises(SystemExit) as error:
        run_cli.run(mode, "--backend", "mlx", *flags)
