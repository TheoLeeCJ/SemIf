...
from semif_phase1.core import load_causal_model, synchronize
...
def test_loader_backend_and_single_device_contract(monkeypatch, backend, count):
    ...
    monkeypatch.setitem(sys.modules, "torch", SimpleNamespace(
        **accelerators, bfloat16="bf16", device=lambda name: name, __version__="test"))
    model = Mock()
    ...
    if count != 1:
        with pytest.raises(ValueError, match="exactly one XPU"):
...
@pytest.mark.parametrize("requested,available,expected_error", [
    ("xpu", ("cuda",), "exactly one XPU"),
    ("cuda", ("xpu",), "exactly one CUDA"),
    ("rocm", ("cuda", "xpu"), "Device must be"),
])
def test_loader_rejects_unavailable_explicit_device(monkeypatch, requested, available, expected_error):
    accelerators = {
...
def test_loader_explicit_device_does_not_prefer_cuda(monkeypatch):
    """An explicit device="xpu" request must not silently fall back to CUDA."""
    accelerators = {
    ...
    monkeypatch.setitem(sys.modules, "transformers", transformers)
    load_causal_model("test/remote", "a" * 40, device="xpu")
...
def test_synchronization_dispatch(monkeypatch):
    ...
    for name in ("cpu", "cuda", "xpu"):
        synchronize(SimpleNamespace(type=name))
